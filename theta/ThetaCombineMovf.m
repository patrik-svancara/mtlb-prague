function [seqtheta,seqpts] = ThetaCombineMovf(movf,frwin,movfind)
% Calculates the movie-specific seqthetas and merges them across the movies.
...
for m = 1:length(movfind)
    ...
    [seqtheta(m,:,:,:),seqpts(m,:)] = ThetaCombineFrames(movf(movfind(m)),frwin);
