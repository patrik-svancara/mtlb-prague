function [gr] = AccToGridPosSingleGroup(group,xbins,ybins,mov)
...
OUT_DATA_TEMPLATE = [outfolder 'grid_pos_acc_data_%s.mat'];
...
for m = 1:length(mov)
    ...
    for t = 1:length(mov(m).tr)
        ...
        % add trajectory if it contains data
        if mov(m).tr(t).length > 0
            
            % position-based grid
            gr = AccFrToGridPos(gr,mov(m).tr(t).pos,mov(m).tr(t).acc);
